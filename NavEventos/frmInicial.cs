...
namespace NavEventos
{
    ...
    {
        #region EVENTOS JANELAS WINDOWS
        ...
        List<string> janelas = new List<string>();
        ...
        private void frmInicial_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            {
                // quando o "enter" para pesquisar registro estiver partindo do Combo ou CNPJ dos fundos, não realiza pesquisa.
                Control ctrl = cUteis.FindFocusedControl((Control)sender); // busca qual controle está em foco
                if (ctrl.Name.Equals("cboFundoOrigem", StringComparison.CurrentCultureIgnoreCase) || ctrl.Name.Equals("txtCNPJCPFOrigem", StringComparison.CurrentCultureIgnoreCase) ||
                    ctrl.Name.Equals("cboFundoDestino", StringComparison.CurrentCultureIgnoreCase) || ctrl.Name.Equals("txtCNPJCPFDestino", StringComparison.CurrentCultureIgnoreCase))
                {
                    e.Handled = true;
                }
                else
        ...
        private void tsbNovo_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                ret_nseq = ce.select_sequencia_evento();
                if (ret_nseq == 0)
                {
                    MessageBox.Show("Não foi possível consultar próximo ID", "Atenção", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    return;
                }
                #endregion  

                txtIDSeq.Text = ret_nseq.ToString();
                ...
                txtCNPJCPFOrigem.Enabled = true;
                txtCNPJCPFDestino.Enabled = true;
        ...
        private void tsbCancelar_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (MessageBox.Show(string.Format("Deseja remover o ID {0}", Convert.ToInt32(txtIDSeq.Text)), "Atenção", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    cMaskara cm = new cMaskara();
                    ...
                    cEvento ev = new cEvento();
                    ev.remove_evento(id_seq);
        ...
        private void tsbSalvar_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (dr == DialogResult.Yes)
                {
                    ...
                    clinha = 70;
                    // Salva evento
                    if (cv.grava_evento(cv) < 1)
                    {
                        MessageBox.Show("Não foi possível incluir o evento", "Atenção", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return;
                    }
                    string msgGravaRegistro = string.Format("A Demanda nº {0}, \nfoi registrada ", int.Parse(txtIDSeq.Text));
                    if (flgRegistroValidado)
                    { 
                        msgGravaRegistro += "e encaminhada ";
                    }
                    msgGravaRegistro += "com sucesso.";

                    MessageBox.Show(msgGravaRegistro, "Mensagem", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    btnSalvaComentario.Enabled = true;

                    cGlobal.infotxt = string.Empty;
                    if (flgRegistroValidado && chklbSetor.SelectedIndex != -1)
                    { 
                        ...
                        string sw = string.Empty;
                            ...
                            sw = "Informações salvas da Demanda:";
                            sw +=  Environment.NewLine;
                            sw += string.Format("ID Demanda: {0}", id);
                            sw += Environment.NewLine;
                            sw += string.Format("Status: {0}", status);
                            sw += Environment.NewLine;
                            sw += string.Format("Razão Social: {0}", nomeFundo);
                            sw += Environment.NewLine;
                            sw += string.Format("Sigla SAC: {0}", siglasac);
                            sw += Environment.NewLine;
                            sw += string.Format("CNPJ/CPF: {0}", cnpjcpf);
                            sw += Environment.NewLine;
                            sw += Environment.NewLine;
                            sw += "EVENTO(S):";
                        clinha = 80;
                        #region PERCORRE TIPOS DE EVENTOS PARA ENVIO DE EMAIL
                            ...
                            for (int i = 0; i < chklbTpEvento.Items.Count; i++)
                            {
                                foreach (DataRow drw in dstp.Tables["TipoEvento"].Rows)
                                {
                                    if (drw["EVENTO"].ToString() == chklbTpEvento.Items[i].ToString())
                                    {
                                        ...
                                        eventos.Add(string.Concat("ID - ", drw["ID_DESCRICAO_EVENTO"].ToString(), " | Evento: ", drw["EVENTO"].ToString(), " | Data Demanda: ", drw["DTDEMANDA"].ToString(), " | Data do Evento: ", drw["DTCOTA"].ToString().Substring(0, 10), Environment.NewLine));
                                        #region Monta Titulo Email
                                        if (string.Compare(setor, "INTRAG") == 0)
                                            tituloEmail = setor + "-" + eventos[indEventos];
                                        else
                                            tituloEmail = eventos[indEventos];
                                        #endregion

                                        assunto = sw + eventos[indEventos];
                                        //   Threads[indEventos] = new Thread(() => (new cEmail()).Monta(tituloEmail, assunto));
                        ...
                        #region GRAVA HISTORICO e COMENTARIO
        ...
        private void dgvDescEvento_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == 4)
            {
                ...
                // Now make it visible  
                if (cGlobal.alterando || navegando)
                {
        ...
        private bool EnumProc(IntPtr hWnd, IntPtr data)
        {
               ...
               ((GetWindowLong(hWnd, -20) & 0x80) == 0))
            {
                ...
                janelas.Add(sb.ToString());
            }
            return true;
        }
        private bool JanelaEmailAberta(string assunto)
        {
            bool encontrou=false;
            EnumWindows(new EnumWindowsProc(EnumProc), IntPtr.Zero);
            janelas.ForEach(delegate (String tituloJanela)
            {
                if (String.Compare(tituloJanela, assunto) == 0)
                {
                    encontrou= true;
                }
                else
        ...
        private void carrega_registros(bool pesquisa)
        {
            try
            {
                ...
                else
                {
                    ...
                    tstxtLocalizar.Enabled = false;
                    tsbAprovacoes.Enabled = false;
                    txtCNPJCPFDestino.Enabled = false;
                    txtCNPJCPFOrigem.Enabled = false;
            ...
            catch (Exception ex)
            {
                ...
                lg.tp_flag = false;
                lg.grava_log(lg);
                MessageBox.Show(lg.log, "Atenção");
                #endregion
            }
        }
        ...
        private void Bloqueio(bool flag)
        {
            ...
            txtCNPJCPFDestino.Enabled = flag;
            txtCNPJCPFOrigem.Enabled = flag;
        }
        ...
        private void retorna_grid_tipo_eventos(int id)
        {
            ...
            try
            {
                ...
                */
                ...
                dgvDescEvento.Columns["RTO"].Frozen = true;
                dgvDescEvento.Columns["RTO"].ReadOnly = true;
...
/*
        ...
        private int grava_comentario(string comentario, string tipo)
        {
            int valret = 0;
            ...
            if (tipo == "COMENTARIO")
            {
                ...
                try
                {
                    ...
                    valret = ce.atualiza_comentario(ce, "COMENTARIOS");
                }
                catch (Exception ex)
                {
                    txtComentario.Text = string.Empty;
            ...
            else if (tipo == "HISTORICO")
            {
                ...
                try
                {
                    ...
                    valret=ce.atualiza_comentario(ce, "HISTORICO");
                }
                catch (Exception ex)
                {
                    throw ex;
        ...
        private void txtCNPJCPFOrigem_KeyPress(object sender, KeyPressEventArgs e)
        {
            try
            {
                if (!Char.IsDigit(e.KeyChar) && e.KeyChar != (char)8)
                ...
                if (e.KeyChar == (char)13)
                {
                    ...
                    ValidacaoFundo(ref cboFundoOrigem, ref txtCNPJCPFOrigem);
                    return;

                }                   
            } catch (Exception ex)
            {
                MessageBox.Show(ex.Message, "Erro ao consultar fundos de Origem");
            }
        }

        private void ValidacaoFundo(ref ComboBox cboFundo, ref TextBox Cnpj)
        {
            try
            {
                    cFundo fn = new cFundo();
                    
                    fn.CnpjCpf = Cnpj.Text.Replace("-", "").Replace(".", "");
                    fn.razao_social = cboFundo.Text;
                    DataSet dsFundo = new DataSet();
                    dsFundo.Merge(fn.find_fundo_CNPJ(fn));
                    if (dsFundo.Tables["RetFundoEvento"].Rows.Count == 0)
                    {
                        DialogResult resp = MessageBox.Show("Fundo não cadastrado. Deseja Incluir?", "Cadastro de Fundo", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                        switch (resp)
                        {
                            case DialogResult.Yes:
                                {
                                    if (string.IsNullOrEmpty(fn.razao_social.ToString()))
                                    {
                                        MessageBox.Show("Razão social deve ser informado!", "Atenção", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                                        cboFundo.Focus();
                                        return;
                                    }
                                    fn.grava_fundo(fn);
                                    carrega_combo_fundo(cboFundo); // atualiza combo
                                    using (dsFundo = fn.find_fundo_CNPJ(fn))
                                    {
                                        cboFundo.SelectedValue = Convert.ToUInt32(dsFundo.Tables["RetFundoEvento"].Rows[0]["ID_FUNDO"].ToString());
                                    }
                                    break;
                                }
                            case DialogResult.No:
                                {
                                    MessageBox.Show("Limpar dados?");
                                    break;
                                }
                        }
                    }
            ...
            {
                MessageBox.Show(ex.Message, "Erro ao consultar fundos");
            }
        }
        private void cboFundoOrigem_KeyPress(object sender, KeyPressEventArgs e)
        {
        ...
        private void cboFundoDestino_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (Char.IsLetter(e.KeyChar))
            {
                e.KeyChar = Char.ToUpper(e.KeyChar);
        ...
        private void txtCNPJCPFDestino_KeyPress(object sender, KeyPressEventArgs e)
        {
            try
            {
                if (!Char.IsDigit(e.KeyChar) && e.KeyChar != (char)8)
                ...
                if (e.KeyChar == (char)13)
                {
                    ...
                    ValidacaoFundo(ref cboFundoDestino, ref txtCNPJCPFDestino);
                    return;
            ...
            {
                MessageBox.Show(ex.Message, "Erro ao consultar fundos de Destino");
            }
        }

        private void rTOToolStripMenuItem_Click(object sender, EventArgs e)
        {


            using (frmFiltroRTO frm = new frmFiltroRTO())
            {
        ...
        private void InserirCalendario()
        {
            DateTime dtinicial =new DateTime(2017,01, 01);
            string sqlInsertCalendario = string.Empty;
            int seqTabela = 1;
            for (int i=0;i<4000;i++)
            {

                if (dtinicial.DayOfWeek.GetHashCode() > 0 && dtinicial.DayOfWeek.GetHashCode() < 6) // se for final de semana
                {
                    sqlInsertCalendario = "Insert into CALENDARIO_TB ( ID, DIA_SEMANA, DIA_ANO, DATAS) VALUES (" +
                     seqTabela + "," + dtinicial.DayOfWeek.GetHashCode() + "," + dtinicial.DayOfYear + ", '" + dtinicial.ToShortDateString().ToString() + "')";
                    seqTabela++;

                    using (OleDbCommand cmd = new OleDbCommand(cmdText: sqlInsertCalendario, connection: cConexao.abre_conexao()))
                    {
        ...
        private bool ehvalidoparaalteracao()
        {
            if (string.Compare(cboStatus.Text,"VALIDAÇÃO GOVERNANÇA")==0)
                return true;
            else
            {
                return false;    
        ...
        private void tsbEditar_Click(object sender, EventArgs e)
        {
            ...
            {
                cGlobal.novo = false;
                cGlobal.alterando = true;
                navegando = false;

                if (!ehvalidoparaalteracao())
                {
                    MessageBox.Show("Evento não pode ser alterado", "atenção", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    return;
                }
            ...
            catch (Exception ex)
            {
                ...
                lg.tp_flag = false;
                lg.grava_log(lg);
                MessageBox.Show(lg.log, "Atenção", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                #endregion
            }
        }
        ...
        private void tsmnuApGovernanca_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                ce.id_seq_evento = int.Parse(txtIDSeq.Text);
                if (ce.atualiza_status_evento(ce, 0) < 1)
                {
                    MessageBox.Show("Status do Evento não atualizado!", "Atenção", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }
        ...
        private void tsmnuApCapacity_Click(object sender, EventArgs e)
        {
                ...
                else
                {
                    ...
                    foreach (DataGridViewRow linha in dgvDescEvento.Rows)
                    {
                        ...
                        foreach (DataGridViewCell item in linha.Cells)
                        {
                            ...
                            {
                              //  MessageBox.Show("== Vai MONTAR PREVIA do cronograma");
                                if (pc.monta_previa_cronograma(int.Parse(txtIDSeq.Text), int.Parse(item.Value.ToString())) < 1)
                                { 
                                    MessageBox.Show("Não foi possível criar cronograma. \nVerifique atividades relacionadas ao Tipo de Evento", "Atenção", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                                    MessageBox.Show("Evento : " + txtIDSeq.Text + "Cod Tipo Evento: " + item.Value.ToString());
                                    user.bloqueia_usuario_capacity(user, false);
                                    return;
                                }
                                c_linha = 24;
                                ...
                                if (pc.verifica_reserva_capacity(int.Parse(txtIDSeq.Text)))
                                {
                                    using (DataSet ds = pc.retorna_data_responsavel_esforco(int.Parse(txtIDSeq.Text), int.Parse(item.Value.ToString())))
                                    {
                                        ...
                                        c_linha = 30;
                                        ce.id_seq_evento = int.Parse(txtIDSeq.Text);
                                        if (ce.atualiza_status_evento(ce, 3)<1)
                                        {
                                            MessageBox.Show("Status do Evento não atualizado","Atenção",MessageBoxButtons.OK,MessageBoxIcon.Error);
                                            return;
                                        }

                                        //ATUALIZA STATUS DA TABELA DESCRICAO_EVENTO_TB
                                        if (ce.atualiza_descricao_evento(true, "Reserva de Capacity.\r\n Necessário enviar para a RTO quando aprovado.", 0, int.Parse(txtIDSeq.Text), 0) < 1)
                                        {
                                            
                                            MessageBox.Show("Descrição do evento não foi atualizada", "Atenção", MessageBoxButtons.OK, MessageBoxIcon.Error);
                                            return;
                                            
                                        }
                                        c_linha = 40;
                                        if (pc.grava_tabela_cronograma(int.Parse(item.Value.ToString()), int.Parse(txtIDSeq.Text.ToString()) ) > 0)
                                        {
                                        ...
                                        {
                                            MessageBox.Show("Não existe atividade cadastrada para este tipo de evento");
                                            return;
                                        }
                                ...
                                else
                                {
                                    ...
                                    string retdtmin = pc.retorna_dt_minima(int.Parse(txtIDSeq.Text), int.Parse(item.Value.ToString()));
                                    if (!string.IsNullOrEmpty(retdtmin) )
                                    {
                                        DateTime dtmin = Convert.ToDateTime(retdtmin.ToString());
                                        //1-REGRA DE VALIDAÇÃO - DENTRO DO PRAZO PARA EXECUÇÃO DO EVENTO
                                        if (dtmin.Date < DateTime.Now.Date)
                                        {
                                            string retNome = pc.retorna_nome_do_evento(int.Parse(item.Value.ToString()));
                                            #region GUARDA LOG
                                            descricao_log = string.Concat("Evento: ",
                                                                          ...
                                                                          retdtmin.ToString().Substring(0, 10)
                                                                         );
                                            ...
                                            //ATUALIZA STATUS DA TABELA DESCRICAO_EVENTO_TB
                                            if (ce.atualiza_descricao_evento(false, string.Concat("Sem Capacity",
                                                                                              Environment.NewLine,
                                                                                              "Não há tempo habil para execução do evento pois existem atividades para execução retroativa: ",
                                                                                              retdtmin.ToString().Substring(0, 10)
                                                                                             ),
                                        ...
                                        {
                                           // MessageBox.Show("Evento é maior que hoje");
                                            ...
                                            using (DataSet ds = pc.retorna_data_responsavel_esforco(int.Parse(txtIDSeq.Text), int.Parse(item.Value.ToString())))
                                            {
                                                ...
                                                foreach (DataRow dr in ds.Tables["RetDtRespEsf"].Rows)
                                                {
                                                    ...
                                                    if (dispT < int.Parse(dr["ESFORCO_PLAN"].ToString()))
                                                    {
                                                        ...
                                                        string retevento = pc.retorna_nome_do_evento(int.Parse(item.Value.ToString()));
                                                        grava_comentario(string.Concat("Proposta Cronograma: ", retnextdt.ToShortDateString(),
                                                                                       Environment.NewLine,
                                                                                       "Evento: ", retevento.ToString(),
                                                                                       Environment.NewLine,
                                                                                       descricao_log
                                                                                       ), "HISTORICO");
                                                        #endregion
                                                        ...
                                                        //ATUALIZA STATUS DA TABELA DESCRICAO_EVENTO_TB
                                                        if (ce.atualiza_descricao_evento(false, string.Concat("Proposta Cronograma: ",
                                                                                            retnextdt.ToShortDateString(),
                                                                                            Environment.NewLine,
                                                                                            descricao_log,
                                                                                            Environment.NewLine),
                                                                                            cGlobal.id_desc_evento,
                                                                                            int.Parse(txtIDSeq.Text),
                                                                                            int.Parse(item.Value.ToString()))<1)
                                                        {
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        ce.id_seq_evento = int.Parse(txtIDSeq.Text);
                                                        if(ce.atualiza_status_evento(ce, 1)<1)
                                                        {
                                                            MessageBox.Show("Status do Evento não atualizado", "Atenção", MessageBoxButtons.OK, MessageBoxIcon.Error);
                                                            return;
                                                        }
                                      ...
                                      //          MessageBox.Show("Vai gerar a TABELA DE CRONOGRAMA");
                                                if (pc.grava_tabela_cronograma(int.Parse(item.Value.ToString()), int.Parse(txtIDSeq.Text.ToString()))>0)
                                                {
                                                    string retnevento = pc.retorna_nome_do_evento(int.Parse(item.Value.ToString()));
                                                    grava_comentario(string.Concat("Cronograma Gerado",
                                                                                   Environment.NewLine,
                                                                                   "Evento: ",
                                                                                   retnevento.ToString()), "HISTORICO");
                                                }
                                                else
                                                {
                                                    MessageBox.Show("Não foi possível criar cronograma. Atividades não encontradas para este tipo de evento.", "Atenção", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                                    return;
                                                }
                                                //ATUALIZA STATUS DA TABELA DESCRICAO_EVENTO_TB
                                                if (ce.atualiza_descricao_evento(false,
                                    ...
                                    {
                                        MessageBox.Show("Problemas ao consultar o registro!", "Atenção", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    ...
                    if (OpenOutlook > 0)
                    {
                        ...
                        string pAssunto = string.Empty;
                        string pCorpo = string.Empty;
                        ...
                        using (DataSet ds = pc.retorna_info_demanda(int.Parse(txtIDSeq.Text)))
                        {
                            ...
                            foreach (DataRow dr in ds.Tables["RetInfDem"].Rows)
                            {
                                ...
                                {
                                    cnpj = (string.Concat(dr["CNPJ_CPF"].ToString().Substring(0, 2), ".",
                                ...
                                {
                                    cnpj = (string.Concat(dr["CNPJ_CPF"].ToString().Substring(0, 3), ".",
                                                             dr["CNPJ_CPF"].ToString().Substring(3, 3), ".",
                                                             dr["CNPJ_CPF"].ToString().Substring(6, 3), "-",
                                                             dr["CNPJ_CPF"].ToString().Substring(9, 2)).Replace(",", "."));
                                }

                                pAssunto = string.Format("Status: {0} | ID DEMANDA: {1} | Sigla SAC: {2} | CNPJ/CPF: {3} ", dr["STATUS"].ToString(), dr["SEQ_EVENTO"].ToString(), dr["SIGLA_SAC"].ToString(), cnpj);
                            }
                        }
                        #endregion
                        c_linha = 100;
                        ...
                        using (DataSet ds = pc.monta_msg_outlook(int.Parse(txtIDSeq.Text)))
                        {
                            ...
                            {
                                string retevento = pc.retorna_nome_do_evento(int.Parse(dr["ID_EVENTO"].ToString()));
                        ...
                        pCorpo = cGlobal.msg_monta;
                        //email.Monta(pAssunto, pCorpo);
                    ...
                    cEvento cv = new cEvento();
                    bool flgIntrag = false;
                    cv.id_seq_evento = Convert.ToInt32(this.txtIDSeq.Text);
                    using (DataSet dsse = ce.retorna_setor_evento(cv))
                    {
                        foreach (DataRow dr in dsse.Tables["SetorEvento"].Rows)
                        {
                           if (dr["Setor"].ToString() == "INTRAG")
                            {
                                flgIntrag = true;
                            }
                        }
                    ...
                    #region Monta corpo de texto do email
                    int idEvento = int.Parse(txtIDSeq.Text);
                    string siglasac = string.Empty;
                    string cnpjcpf = string.Empty;
                    string nomeFundo = string.Empty;
                    using (DataSet dsfd = cv.retorna_dados_fundo_origem(cv))
                    ...
                    corpoEmail = "Informações salvas da Demanda:";
                    corpoEmail += Environment.NewLine;
                    corpoEmail += string.Format("ID Demanda: {0}", idEvento);
                    corpoEmail += Environment.NewLine;
                    corpoEmail += string.Format("Status: {0}", "VALIDACAO CAPACITY");
                    corpoEmail += Environment.NewLine;
                    corpoEmail += string.Format("Razão Social: {0}", nomeFundo);
                    corpoEmail += Environment.NewLine;
                    corpoEmail += string.Format("Sigla SAC: {0}", siglasac);
                    corpoEmail += Environment.NewLine;
                    corpoEmail += string.Format("CNPJ/CPF: {0}", cnpjcpf);
                    corpoEmail += Environment.NewLine;
                    corpoEmail += Environment.NewLine;
                    corpoEmail += "EVENTO(S):";
                    corpoEmail += Environment.NewLine;
                        ...
                        for (int i = 0; i < chklbTpEvento.Items.Count; i++)
                        {
                            foreach (DataRow drw in dstp.Tables["TipoEvento"].Rows)
                            {
                                if (drw["EVENTO"].ToString() == chklbTpEvento.Items[i].ToString())
                                {
                                    ...
                                    eventos = string.Concat("ID - ", drw["ID_DESCRICAO_EVENTO"].ToString(), " | Evento: ", drw["EVENTO"].ToString(), " | Data Demanda: ", drw["DTDEMANDA"].ToString(), " | Data do Evento: ", drw["DTCOTA"].ToString().Substring(0, 10), Environment.NewLine);
                                    #region Monta Titulo Email
                                    if (flgIntrag)
                                        eventos = "** INTRAG ** - " + eventos;
                                    #endregion
                                    int idTipoEvento = Convert.ToInt32(drw["ID_DESCRICAO_EVENTO"]);
                                    pc.grava_mensagem_email(idEvento, idTipoEvento, eventos);

                                    // a variavel sw contém o texto do email. Os eventos são acrescentados no texto do corpo do email dentro da rotina.
                                    (new cEmail()).Monta(idEvento, corpoEmail) ;
                    ...
                    if (qtd_cron == 0 && !mostrar_msg)
                    {
                        ...
                        MessageBox.Show("Região do erro: " + c_linha);
                        MessageBox.Show("GErou cronograma:" + qtd_cron);
                        this.Cursor = Cursors.Arrow;
                        return;
                    }
                    else
            ...
            catch (Exception ex)
            {
                ...
                #region LOG ERRO
                cLog lg = new cLog();
                lg.log = ex.Message.Replace("'", "") +  " na linha " + c_linha;
                lg.form = this.Text;
                lg.metodo = sender.ToString();
                lg.dt = DateTime.Now;
                lg.usersistema = cGlobal.userlogado;
        ...
        private void tsmnuApSuporte_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                ce.id_seq_evento = int.Parse(txtIDSeq.Text);
                if (ce.atualiza_status_evento(ce, 4)<1)
                {
                    MessageBox.Show("Status do Evento não atualizado", "Atenção", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
